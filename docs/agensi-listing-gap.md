...
而已上线的官网（`workspace/src/config/site.ts` 第 22–27 行）卖的是**一个 USD 19.90 的整包**，六个 skill 全含。
...
| 与官网的关系 | 价格阶梯（USD 19 / 15 / 15 / 9 / 9 / 9）官网完全没有体现，官网只有 USD 19.90 | 与官网价位口径接近，但仍需决定 Agensi 上写 USD 19.90 还是别的数 |
...
**两个渠道并不互斥**：Agensi 按单个 skill 卖、官网卖整包，在机制上完全可以并存。但**同时看到两边的买家会做算术**：Agensi 六个加起来是 USD 76（19+15+15+9+9+9），官网整包 USD 19.90。这个 3.8 倍的落差如果没有一句解释，会同时伤害两边 —— 在 Agensi 显得官网在倾销，在官网显得 Agensi 在宰客。

**建议**：采用 §1.3(a) 的六个独立上架（技术理由——避免二层嵌套导致安装失败——是硬的，不是偏好问题），但**必须同时把官网的 USD 19.90 重新定位成"六合一套装价"并在官网明写"单买请到 Agensi"**，让 USD 76 → USD 19.90 变成一个可解释的套装折扣，而不是一处矛盾。具体折扣叙述怎么写属于营销决策，不在 PUBLISHING.md 覆盖范围内。
...
| # | 步骤 | 依赖 | 粗略工作量 |
| --- | --- | --- | --- |
| 1 | **拍板 §3.1 定价与上架形态**：确认六个独立上架 + 官网 USD 19.90 重新定位为套装价，并定下官网侧的说明措辞 | 无 | 半天思考，不含官网改文案 |
...
| 10 | **官网侧收尾**：按第 1 步定的口径更新 USD 19.90 的套装说明，加"单买请到 Agensi"指向 `agensiListingUrl` | 8 | 2 小时 |
...
| USD 19.90、CROSSXTOP 四项法人信息、Agensi 链接 | `/Users/mac/workspace/TeachFlowSkills/workspace/src/config/site.ts` |
