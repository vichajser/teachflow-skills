...
本期**仅做展示**，站内不实现结算。后端与结算能力在目录结构上预留，不实现。

`/pricing` 须并列呈现两条购买路径，且**直销路径不得弱于 Agensi 路径**（见 10.3 的审核风险）：

1. **Agensi 商城** —— 跳转商品页，即时下载
2. **直接购买** —— 邮件 `crossxtop@gmail.com`，人工开票与交付

本站 Stripe 账户对应的是第 2 条路径的收款。

### 1.2 定价

单一 SKU：**TeachFlow-KR Complete Bundle，USD 29.90，一次性买断**，包含全部 6 个 skill。

全站价格一律写作 `USD 29.90`（含币种代码），不写 `$29.9`。Stripe 要求购买币种无歧义。
...
- UI 文案集中于 `src/i18n/{en,ko}.json`，不散落在组件中
- 长文内容用 Astro 内容集合，按语言分目录
- 每页输出 `hreflang`（en / ko + `x-default` 指向 en）与 `canonical`
- 语言切换保持当前路径，不回首页
...
- 移动端自动切竖排布局
- 所有标签走 i18n 变量，不烧进 SVG；节点宽度用 `textLength` 自适应，避免韩语标签溢出
...
| EN | Inter |
| KO | Pretendard |
| Mono | JetBrains Mono（两语共用） |

通过 `:lang()` 自动切换，自托管 woff2 + `font-display: swap`。

韩语在同字号下视觉重量大于英文，韩语标题字号下调一档。

### 5.5 合规区静音

`/pricing` 与 `/legal/*` 沿用同一色板，但正文区改用 `--bg-surface` 高对比阅读版式，**取消所有辉光与动效**，价格 `USD 29.90` 以大号高对比呈现。
...
| 要求 | 位置 | 内容 |
|---|---|---|
| 全部在售商品 + 价格 | `/pricing` | TeachFlow-KR Complete Bundle — USD 29.90，一次性买断，列全 6 个 skill |
| 具体商品描述 | `/pricing` + `/skills` | 6 个 Claude Skill 文件包（SKILL.md + references），zip 下载 |
| 数字商品交付说明 | `/legal/delivery` | **两条路径分别说明**。经 Agensi：付款后获签名下载链接（24 小时有效，dashboard 可重新生成），zip 含 SKILL.md + references，版本更新免费重下。直销：收款确认后 2 个工作日内邮件发送下载链接。均无实体配送。另列明运行环境要求 |
| 退款政策 | `/legal/refund` | 见 6.2 |
| 取消政策 | `/legal/refund` 同页 | 一次性买断、无订阅、无自动续费 |
| 客服联系方式 | 全站页脚 + `/pricing` 页内"联系与支持"区块 | `crossxtop@gmail.com` 直达 + 响应时限承诺（建议"2 个工作日内"）；不得仅提供表单。不单独开 `/support` 页 |
| 主体信息一致 | 全站页脚 | 见 3.3 |
...
- **韩国消费者** → 「전자상거래 등에서의 소비자보호에 관한 법률」第 17 条 청약철회：自收到第 13 条第 2 项书面合同内容之日（若供给晚于书面，则自供给开始之日）起 **7 日**；当事人约定更长期间的，从该期间（直销 14 日优先）。数字内容开始提供后，可依第 17 条第 2 项第 5 号限制撤回，但销售者未做第 17 条第 6 项及施行令第 21 条之 2 措施时，7 日权利仍在。货不对板走第 17 条第 3 项（供给日起 3 个月 **且** 知悉日起 30 日）。撤回后第 18 条第 2 项 3 个营业日内退款，第 18 条第 9 项禁止违约金。法令名在英韩页面均保留韩文原名以便检索，官方原文链到 `law.go.kr` lsId=009318。

EU/UK 一条是英国主体向 EU/UK 消费者销售数字商品的法定要求，不可省略。韩国一条因主受众是韩国教师，写在同一页。韩国 7 日是**我们自己的法定义务告知**，不是复述 Agensi 的退款窗口。

**写法纪律**：第一条**只给链接、不复述 Agensi 的具体退款天数**。Agensi 自家两份文件互相矛盾（`/terms` §5.5 称 30 天无理由，`/stripe-terms` 称 14 天且需"materially defective"），且其条款可随时变更。复述等于把别人的错误抄进我们的法律页。韩国法定的 7 日、3 个月、30 日也**不得写进 Agensi 那一节**，只放在「대한민국 소비자이신 경우」节。
...
"可验证方式"一列填写用户自查路径。两条最有力的可验证事实（据 README §8）：

- **包内无可执行代码** —— 全部为 32 个 Markdown 文档，用户可逐个打开通读全部指令
- **自带 `verify.py` 完整性校验** —— 用户可自行运行，输出 `[PASS]` 即未被篡改
...
`sources/Think1_Unit07.pdf` + `sources/T1_U07_TG.pdf`（Teaching Guide）。教材本身为英语教材，符合韩国英语课实际。

产物的语言分配**并非本设计的选择，而是产品既有规则**（README §6 FAQ「한국어로 나오는 부분과 영어로 나오는 부분이 정해져 있나요?」）：

> 教师阅读的内容（수업 지도안、규칙 설명、채점 기준、학부모 안내문 본문）为韩语；课堂使用的语言（Teacher Talk、문장 지원 틀、学生可见的幻灯片文字、학습지 문항）为英语。
...
- **客服邮箱** —— `crossxtop@gmail.com`（域名确定后建议迁移至自有域名）
...
| Bundle 商品 | **平台原生支持**。创作者可把多个 skill 组合成一个合并价商品；购买 bundle 即取得其中全部 skill 的所有权，许可条款相同。已持有其中若干个的买家可享减价。`/bundles` 已有 220 个在售，卡片显示组合价、划掉的单品合计与 `Save $X · Y%` | `/terms` §5.4；`/bundles`（2026-09-19 核实） |
| 最低价 | 付费 skill 不得低于 **USD 5.00** | `/sell` |

按 70% 计，USD 29.90 的单笔 bundle 成交实收约 **USD 20.93**。

本站售卖形态据此定为：六个 skill 在 Agensi 各自独立上架，**并**在其上架一个含全部六个的
bundle，定价 USD 29.90 —— 即官网唯一显示的那个数。详见
`docs/agensi-listing-gap.md` §3.1。
...
**已采用方案（用户 2026-09-18 确认）**：`/pricing` 明确区分两条购买路径——经 Agensi 购买，或直接联系 `crossxtop@gmail.com` 购买；`/legal/refund` 对直销路径给出本站自己的 14 天条款。本站 Stripe 账户对应的即为直销收款。此方案不扩大本期"仅做展示"的范围。

未采用：本期实现站内结算（超出范围）、推迟 Stripe 申请。
